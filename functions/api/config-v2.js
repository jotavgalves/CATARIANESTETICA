...
function admin(request,env){
  ...
  let token=encodeURIComponent(env.ADMIN_SESSION_SECRET||env.ADMIN_PASSWORD||'');
...
export async function onRequestPut({request,env}){if(!admin(request,env))return json({error:'Não autorizado. Faça login no /adm antes de salvar.'},401);if(!env.SITE_CONFIG)return json({error:'Configure SITE_CONFIG no Cloudflare'},500);let body=await request.json();let config=body.config||body;config.version=2;await env.SITE_CONFIG.put('site-config-v2',JSON.stringify(config));return json({ok:true,config})}
