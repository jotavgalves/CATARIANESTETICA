...
async function fallback(request){try{let r=await fetch(new URL('/config/site.v2.json',request.url));if(r.ok)return await r.json()}catch{}return {site:{name:'Catarina Queiroz'},theme:{colors:{},layout:{}},sections:{}}
}
...
export async function onRequestPut({request,env}){if(!admin(request))return json({error:'Não autorizado'},401);if(!env.SITE_CONFIG)return json({error:'Configure SITE_CONFIG no Cloudflare'},500);let body=await request.json();let config=body.config||body;await env.SITE_CONFIG.put('site-config',JSON.stringify(config));return json({ok:true,config})}
