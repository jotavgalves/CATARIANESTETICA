...
const whatsappIcon = '<img class="whatsapp-icon" src="/assets/icons/whatsapp.svg" alt="" aria-hidden="true">';
...
export function renderPublicSite(data: PublicSitePayload): string {
  ...
  const navigation = sections
    .filter((item) => item.is_enabled && renderableSections.has(item.section_key))
    .map((item) => `<a href="#${esc(item.section_key)}">${esc(navigationLabels[item.section_key] || item.title)}</a>`)
    .join("");
  const logoUrl = url(settings.logo_url);
  const brandVisual = `<span class="brand-visual">${logoUrl ? `<img class="brand-logo" src="${logoUrl}" alt="" data-logo-image>` : ""}<span class="brand-monogram" data-logo-fallback>CQ</span></span>`;
  const brand = `<a class="brand" href="#inicio" aria-label="${esc(data.site.name)}">${brandVisual}<span class="brand-copy"><strong>${esc(settings.professional_name)}</strong><span>Clínica Estética</span></span></a>`;
  ...
  return `
    <header class="site-header" data-header><div class="nav-shell">${brand}<nav class="nav-links" data-nav aria-label="Navegação principal">${navigation}<a class="button button-primary header-cta" href="${whatsappUrl}" target="_blank" rel="noopener" data-track="click_whatsapp" data-placement="header">${whatsappIcon}<span>Agendar avaliação</span></a></nav><button class="menu-button" type="button" data-menu aria-label="Abrir menu" aria-expanded="false"><span></span><span></span><span></span></button></div></header>
    <main id="main">
      <section class="hero" id="inicio"><img class="hero-media" src="${url(hero.image_url)}" alt="Atendimento estético em ambiente profissional"><div class="hero-overlay"></div><div class="container hero-content"><p class="eyebrow">${esc(hero.eyebrow)}</p><h1>${esc(hero.title)}</h1><p class="lead">${esc(hero.subtitle)}</p><div class="hero-actions"><a class="button button-light" href="${whatsappUrl}" target="_blank" rel="noopener" data-track="click_whatsapp" data-placement="hero">${whatsappIcon}<span>${esc(hero.primary_cta || "Agendar pelo WhatsApp")}</span></a><a class="button button-outline" href="#procedures">${esc(hero.secondary_cta || "Conhecer os procedimentos")}</a></div><div class="hero-trust"><span>Atendimento com hora marcada</span><span>Avaliação antes da indicação</span><span>${esc(city)}</span></div></div></section>
